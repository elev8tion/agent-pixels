...
// Result card for done action - matches floating panel's doneSuccess / doneError
...
function ResultCard({
	...
	return (
		<div
			className={cn(
				'rounded-xl border p-[11px] relative overflow-hidden',
				success 
					? 'border-[var(--success)]/35 bg-[linear-gradient(135deg,rgba(34,197,94,0.22),rgba(34,197,94,0.09))] shadow-[0_3px_10px_rgba(34,197,94,0.2)]' 
					: 'border-destructive/35 bg-[linear-gradient(135deg,rgba(239,68,68,0.22),rgba(239,68,68,0.08))] shadow-[0_3px_10px_rgba(239,68,68,0.2)]'
			)}
		>
			<div className="flex items-center gap-2 mb-1.5">
				...
				<span
					className={cn(
						...
						success ? 'text-[var(--success)]' : 'text-destructive'
					)}
				>
					{success ? 'SUCCESS' : 'FAILED'}
				</span>
			</div>
			<p className="text-[11px] text-foreground/90 pl-5 whitespace-pre-wrap leading-snug">{text}</p>
			{children}
		</div>
	)
}
...
function CopyButton({ text, label }: { text: string; label: string }) {
	...
	return (
		<button
			...
			className="text-[9px] text-muted-foreground hover:text-foreground transition-colors border px-1 rounded-md shrink-0 cursor-pointer bg-background/50 hover:bg-background/80"
		>
			{copied ? 'Copied!' : label}
		</button>
	)
...
function StepCard({ event }: { event: AgentStepEvent }) {
	return (
		<div className="rounded-xl border-l-[3px] border-l-[var(--accent-cyan)] border bg-muted/30 p-2.5 glass">
			<div className="text-[10px] font-semibold text-foreground/80 tracking-[0.5px] uppercase mb-1.5">
			...
			{event.action && (
				<div>
					<div className="text-[10px] font-semibold text-foreground/70 tracking-[0.5px] uppercase mb-1">
						Actions
					</div>
...
function ObservationCard({ event }: { event: ObservationEvent }) {
	return (
		<div className="rounded-xl border-l-[3px] border-l-[var(--success)] border bg-muted/30 p-2.5 glass">
			{/* <div className="text-[11px] font-semibold text-foreground uppercase tracking-wide mb-2">
				Observation
			</div> */}
			<div className="flex items-start gap-2">
...
function RetryCard({ event }: { event: RetryEvent }) {
	return (
		<div className="rounded-xl border border-[var(--accent-yellow)]/40 bg-[var(--accent-yellow)]/8 p-2.5">
...
export function ActivityCard({ activity }: { activity: AgentActivity }) {
	...
	return (
		<div className="flex items-center gap-2 rounded-xl border glass bg-muted/30 p-2.5">
			...
			<span className={cn('text-[11px] font-medium', info.color)}>{info.text}</span>
		</div>
	)
}

...
import { GlassNavbar } from '@/components/misc'
...
export function HistoryList({
	...
	return (
		<div className="flex flex-col h-screen bg-background rounded-t-[12px]">
			{/* Header */}
			<GlassNavbar
				left={
					<div className="flex items-center gap-2">
						...
						<span className="text-sm font-semibold tracking-[-0.1px]">History</span>
					</div>
				}
				right={
					sessions.length > 0 && (
			...
			<div className="flex-1 overflow-y-auto">
				...
				{sessions.map((session) => (
					<div
						...
						className="w-full text-left px-3 py-2.5 border-b hover:bg-muted/30 active:bg-muted/50 transition-all cursor-pointer flex items-start gap-2 group border-l-2 border-l-transparent hover:border-l-[var(--accent-cyan)]/40"
					>
						{/* Status icon */}
						...
						<div className="flex-1 min-w-0">
							<p className="text-[12px] font-medium truncate leading-tight">{session.task}</p>
